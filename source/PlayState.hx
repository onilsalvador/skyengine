...
class PlayState extends MusicBeatState
{
	...
	public var singarrowp1:String = '';
	public var singarrowp2:String = '';
	public var winningp1:String = '';
	public var winningp2:String = '';
	...
	//sky engine variables
	public var titleUpdate:Bool = true;
	...
	override public function create()
	{
		...
		startCharacterPos(dad, true, iconP2, true);
		dadGroup.add(dad);
		trace('dad added');
		boyfriend = new Boyfriend(0, 0, SONG.player1);
		iconP1 = new HealthIcon(boyfriend.healthIcon, true);
		startCharacterPos(boyfriend, true, iconP1);
		...
		switch(type) {
			case 0:
				if(!boyfriendMap.exists(newCharacter)) {
					...
					var newicon:HealthIcon = new HealthIcon(newBoyfriend.healthIcon, newBoyfriend.isPlayer);
					boyfriendMap.set(newCharacter, newBoyfriend);
					boyfriendGroup.add(newBoyfriend);
					startCharacterPos(newBoyfriend, true, newicon);
			...
			case 1:
				if(!dadMap.exists(newCharacter)) {
					...
					var newicon:HealthIcon = new HealthIcon(newDad.healthIcon, newDad.isPlayer);
					dadMap.set(newCharacter, newDad);
					dadGroup.add(newDad);
					startCharacterPos(newDad, true, newicon, true);
			...
			case 2:
				if(!gfMap.exists(newCharacter)) {
					...
					gfGroup.add(newGf);
					startCharacterPos(newGf, false);
					newGf.alpha = 0.00001;
					newGf.alreadyLoaded = false;
				}
		}
	}
	function startCharacterPos(char:Character, hasicon:Bool, ?icon:HealthIcon, ?gfCheck:Bool = false) {
		if(gfCheck && char.curCharacter.startsWith('gf')) { //IF DAD IS GIRLFRIEND, HE GOES TO HER POSITION
			char.setPosition(GF_X, GF_Y);
			char.scrollFactor.set(0.95, 0.95);
		}
		trace('changing charpos added');
		char.x += char.positionArray[0];
		char.y += char.positionArray[1];
		if (icon != null){
			trace('changing ico pos');
		icon.x += icon.json.position[0];
		icon.y += icon.json.position[1];
		}
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (titleUpdate)
			Application.current.window.title = "Friday Night Funkin' Sky Engine | " + 'Score: ' + songScore + ' | Misses: ' + songMisses + ' | Rating: ' + ratingString;
		
		if (dad.floating) 
			{
				floatfloat += dad.floatingvalue;
				dad.y += Math.sin(floatfloat);
			}
		if (boyfriend.floating) 
			{
				floatfloat += boyfriend.floatingvalue;
				boyfriend.y = Math.sin(floatfloat);
			}
		...
		switch (iconP1.json.icontype)
		{
			case 2:
			{
				iconP1.animation.play(iconP1.name + winningp1 + singarrowp1);
		...
		switch (iconP2.json.icontype)
			{
				case 2:
				{
					iconP2.playAnim(iconP2.name + winningp2 + singarrowp2);
					if (healthBar.percent < 80 && healthBar.percent > 20)
						{
							winningp2 = '';
						}
...
public function checkEventNote() 
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Change Window Icon And Title':

				titleUpdate = false;
				var title:String = value1;
				Application.current.window.title = title;
				if (value2 != null || value2 != '')
				Application.current.window.setIcon(SkyPath.getAppIcon(value2));
			
			case 'Set Original Title And Icon':
				
				titleUpdate = true;

			case 'Temporal Trail':
				var char:Int = Std.parseInt(value1);
				var value:Int = Std.parseInt(value2);
				var charbftrail = false;
				var bftrail:FlxTrail = null;
				var gftrail:FlxTrail = null;
				var dadtrail:FlxTrail = null;
				var chardadtrail = false;
				var chargftrail = false;
				if(Math.isNaN(char)) char = 1;
				if(char > 2 || char < 0) char = 1;
				switch (char) {
					case 0: 
						if (!charbftrail)
						{
						bftrail = new FlxTrail(boyfriend, null, 4, 24, 0.3, 0.069); //nice one
						insert(members.indexOf(boyfriendGroup) - 1, bftrail);
						charbftrail = true;
						}
						if (value == 1) { if (bftrail != null) {bftrail.destroy(); bftrail = null;}
						}
					case 1:
						if (!chardadtrail || dad.curCharacter != 'spirit')
							{
						dadtrail = new FlxTrail(dad, null, 4, 24, 0.3, 0.069); //really nice
						insert(members.indexOf(dadGroup) - 1, dadtrail);
						chardadtrail = true;
						}
						if (value == 1) { if (dadtrail != null) {dadtrail.destroy(); dadtrail = null;}
						}
					case 2:
						if (!chargftrail)
							{
						gftrail = new FlxTrail(gf, null, 4, 24, 0.3, 0.069); //oh la-ti-da
						insert(members.indexOf(gfGroup) - 1, gftrail);
						chargftrail = true;
						}
						if (value == 1) { if (gftrail != null) {gftrail.destroy(); gftrail = null;}
						}
				}
		...
		if (!boyfriend.stunned)
		{
			...
			switch (direction)
			{
				case 0:
					...
					singarrowp1 = 'singLEFTmiss';
				case 1:
					boyfriend.playAnim('singDOWNmiss', true);
					singarrowp1 = 'singDOWNmiss';
				case 2:
					boyfriend.playAnim('singUPmiss', true);
					singarrowp1 = 'singUPmiss';
				case 3:
					boyfriend.playAnim('singRIGHTmiss', true);
					singarrowp1 = 'singRIGHTmiss';
			}
			vocals.volume = 0;
		}
	}
