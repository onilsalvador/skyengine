...
class CharacterEditorState extends MusicBeatState
{
	...
	var charfloat:FlxUICheckBox;
	var charfloatvalue:FlxUINumericStepper;

	function addExtraUI() {
		var tab_group = new FlxUI(null, UI_box);
		tab_group.name = "Extra";

		charfloat = new FlxUICheckBox(15, iconSingingreal.y + 20, null, null, "Character Floating", 80);
		charfloat.checked = char.floating;
		charfloat.callback = function() {
			char.floating = charfloat.checked;
		};

		charfloatvalue = new FlxUINumericStepper(charfloat.x + 110, charfloat.y, 0.01, char.floatingvalue, -99, 99, 2);

		tab_group.add(iconSingingreal);
		tab_group.add(charfloat);
		tab_group.add(charfloatvalue);
	...
	var iconSingingreal:FlxUICheckBox;
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			else if(sender == charfloatvalue)
			{
				char.floatingvalue = charfloatvalue.value;
				updatePointerPos();
	...
	function reloadCharacterOptions() {
		if(UI_characterbox != null) {
			...
			noAntialiasingCheckBox.checked = char.noAntialiasing;
			charfloat.checked = char.floating;
			charfloatvalue.value = char.floatingvalue;
			resetHealthBarColor();
			leHealthIcon.changeIcon(healthIconInputText.text);
	...
	function reloadCharacterDropDown() {
		...
		#if MODS_ALLOWED
		characterList = [];
		var directories:Array<String> = [modloader.ModMenu.curmodfolder + 'characters/', Paths.getPreloadPath('characters/')];
	...
	override function update(elapsed:Float)
	{
		if (char.floating)
		{
			floatfloat += char.floatingvalue;
			char.y += Math.sin(floatfloat);
			updatePointerPos();
	...
	function saveCharacter() {
		var json = {
			...
			"char_float": char.floating,
			"char_floatv": char.floatingvalue,
